        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h4>Activities having dropdown question <a href="{{route('dropdown.create')}}"><button class = "btn btn-primary float-end">Dropdown Create</button></a></h4>
...
{{--                                <th>Project</th>--}}
                            ...
                            <tbody>
                            @foreach($acitivityWithDropdownQuestions as $activity)
                            <tr>
                                <td>{{$loop->iteration}}</td>
{{--                                <td>{{$template->getProject->getUnit->getZone->getCompany->company_name}}</td>--}}
{{--                                <td>{{$template->getProject->getUnit->getZone->zone_name}}</td>--}}
{{--                                <td>{{$template->getProject->getUnit->unit_name}}</td>--}}
{{--                                <td>{{$template->getProject->project_name}}</td>--}}
                        ...
                        <div>
                            {{$acitivityWithDropdownQuestions->links()}}
                        </div>
                    </div>
                </div>
            </div>
