            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Update Zone <a href="{{route('unit.list')}}"><button class = "btn btn-primary float-end">View Unit</button></a></h4>
                    </div>
                    <div class="card-body">
                                ...
                                <div class="col-12">
                                    <div class="tab-content dark-field" id="horizontal-wizard-tabContent">
                                        <div class="tab-pane fade show active" id="wizard-info" role="tabpanel" aria-labelledby="wizard-info-tab">
                                            <form method="POST" action="{{route('unit.update', ['id' =>$unit->id])}}" enctype="multipart/form-data" class="row g-3 needs-validation" novalidate="">
                                                @csrf
                                                @method("PUT")
                                                <div class="col-xl-4 col-sm-4">
                                                    ...
                                                    <select class="form-select" id="company_id" name="company_id" required="">
                                                        ...
                                                        @foreach($companies as $company)
                                                            <option value="{{$company->id}}" @if($unit->company_id == $company->id) selected @endif>{{$company->company_name}}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('company_id')
                                                ...
                                                <div class="col-xl-4 col-sm-4">
                                                    ...
                                                    <select class="form-select" id="zone_id" name="zone_id" required="">
                                                        ...
                                                        @foreach($zones as $zone)
                                                            <option value="{{$zone->id}}" @if($unit->zone_id == $zone->id) selected @endif>{{$zone->zone_name}}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('zone_id')
                                                    <p class="text-red-500 text-xs mt-1">
                                                ...
                                                <div class="col-xl-4 col-sm-4">
                                                    ...
                                                    <input class="form-control" id="unit_name" value="{{$unit->unit_name}}" name="unit_name" type="text" placeholder="Enter unit name" required="">
                                                    @error('unit_name')
                                                    <p class="text-red-500 text-xs mt-1">
                                                        {{$message}}
                                                    </p>
                                                ...
                                                <div class="col-xl-6 col-sm-6">
                                                    ...
                                                    <input class="form-control" id="unit_code" value="{{$unit->unit_code}}" name="unit_code" type="text" placeholder="Enter Short Code" required="">
                                                    @error('unit_code')
                                                    <p class="text-red-500 text-xs mt-1">
                                                        {{$message}}
                                                    </p>
