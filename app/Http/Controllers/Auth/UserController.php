...
//            dd($check_data_assign);
            ...
            if (isset($request->template_name_head_values) && !empty($request->template_name_head_values)) {
                                    ...
                                    foreach ($childProjectTemplate as $childProject) {
                                        ...
                                        if ($childProject->activityType == 0) {
                                            ...
                                            if (!empty($childRowIds)) {
                                                ...
                                                $this->AssignDataWithoutTemplateData($dataAssign->id, $auditor_id, $childProject->activity_group_name_id_or_activity_id, $childProject->id, $act_group_id);
                                            }
                ...
                if (!empty($request->auditor_names)) {
                    foreach ($request->auditor_names as $auditor_id) {
                        $this->AssignDataWithoutTemplateData($dataAssign->id, $auditor_id, $selected_activities, $project_template_id->id, $act_group_id);
                        if ($is_outlet_assinged == 1) {
                            $childProjectTemplate = DB::table('project_templates')
                                ->where('project_id', $project_template_id->project_id)
                                ->where('is_master', 0)
                            ...
                            if (!empty($childProjectTemplate)) {
                                foreach ($childProjectTemplate as $childProject) {
                                    if ($childProject->activityType == 0) {
                                        ...
                                        foreach ($getActivityIds as $childActivityId) {
                                            // Assign child template rows (same logic, different template)
                                            $this->AssignDataWithoutTemplateData($dataAssign->id, $auditor_id, $childActivityId, $childProject->id, $childProject->activity_group_name_id_or_activity_id);
                                        }
                                    }
                                }
                            }
    ...
    public function AssignDataWithoutTemplateData($dataAssignId, $auditorId, $activtyId, $project_template_id, $act_group_id = null)
    {
        ...
        if (!$commonId) {
            ...
            if ($act_group_id) {
                $data['activity_sequence_id'] = ActivityGroupPivot::where('activity_group_id', $act_group_id)
                    ->where('activity_id', $activtyId)
                    ->value('sequence');
            }
